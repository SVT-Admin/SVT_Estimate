...
function generateProfessionalBillPDF(bill) {
    ...
    const template = `
        <div id="bill-pdf-content" style="padding: 20px; font-family: Arial, sans-serif; width: 210mm;">
            <!-- Header Section -->
            <div style="text-align: center; margin-bottom: 20px;">
                <h1 style="margin: 0; font-size: 24px; color: #000;">SRI VINAYAGA TRADERS</h1>
                <p style="margin: 5px 0; font-size: 12px;">123 Main Street, First Floor</p>
                <p style="margin: 5px 0; font-size: 12px;">City Name, State - PIN Code</p>
                <p style="margin: 5px 0; font-size: 12px;">Phone: +91 1234567890, +91 9876543210</p>
            </div>

            <h2 style="font-size: 18px; text-align: center; margin: 15px 0;">ESTIMATE</h2>

            <!-- Bill Info Section -->
            <table style="width: 100%; margin-bottom: 15px; font-size: 12px;">
                <tr>
                    <td style="width: 33%;"><strong>Estimate No:</strong> ${bill.billNumber}</td>
                    <td style="width: 33%; text-align: center;"><strong>Date:</strong> ${new Date(bill.date).toLocaleDateString()}</td>
                    <td style="width: 33%; text-align: right;"><strong>Time:</strong> ${new Date(bill.date).toLocaleTimeString()}</td>
                </tr>
            </table>

            <!-- Customer & Staff Details -->
            <div style="margin-bottom: 15px; font-size: 12px;">
                <div style="float: left; width: 48%; border: 1px solid #ddd; padding: 5px;">
                    <h3 style="margin: 0 0 5px 0; font-size: 14px;">Customer Details</h3>
                    <p style="margin: 3px 0;"><strong>Name:</strong> ${bill.customer.name}</p>
                    <p style="margin: 3px 0;"><strong>Mobile:</strong> ${bill.customer.mobile}</p>
                    <p style="margin: 3px 0;"><strong>Address:</strong> ${bill.customer.address}</p>
                </div>
                <div style="float: right; width: 48%; border: 1px solid #ddd; padding: 5px;">
                    <h3 style="margin: 0 0 5px 0; font-size: 14px;">Staff Details</h3>
                    <p style="margin: 3px 0;"><strong>Name:</strong> ${bill.staff.name}</p>
                    <p style="margin: 3px 0;"><strong>Role:</strong> ${bill.staff.role}</p>
                </div>
            ...
            <table style="width: 100%; border-collapse: collapse; margin-bottom: 15px; font-size: 12px;">
                <thead>
                    <tr style="background-color: #f8f9fa;">
                        <th style="border: 1px solid #ddd; padding: 5px; text-align: center;">S.No</th>
                        ...
                        <th style="border: 1px solid #ddd; padding: 5px; text-align: center;">Qty (KG)</th>
                        <th style="border: 1px solid #ddd; padding: 5px; text-align: right;">Price/KG</th>
                ...
                <tbody>
                    ${bill.items.map((item, index) => `
                        <tr>
                            ...
                            <td style="border: 1px solid #ddd; padding: 5px;">${item.brandName}</td>
                            ...
                            <td style="border: 1px solid #ddd; padding: 5px; text-align: right;">₹${item.price.toFixed(2)}</td>
                            <td style="border: 1px solid #ddd; padding: 5px; text-align: right;">₹${(item.quantity * item.price).toFixed(2)}</td>
                        </tr>
                    `).join('')}
                </tbody>
            </table>
            ...
            <div style="width: 100%; font-size: 12px;">
                <table style="width: 300px; margin-left: auto;">
                    <tr>
                        <td style="padding: 3px;"><strong>Sub Total:</strong></td>
                        <td style="text-align: right; padding: 3px;">₹${bill.subtotal.toFixed(2)}</td>
                    </tr>
                    <tr>
                        <td style="padding: 3px;"><strong>GST (${bill.gstPercentage}%):</strong></td>
                        <td style="text-align: right; padding: 3px;">₹${bill.gstAmount.toFixed(2)}</td>
                    </tr>
                    <tr>
                        <td style="padding: 3px;"><strong>Transport Charges:</strong></td>
                        <td style="text-align: right; padding: 3px;">₹${(bill.transportCharges || 0).toFixed(2)}</td>
                    </tr>
                    <tr>
                        <td style="padding: 3px;"><strong>Extra Charges:</strong></td>
                        <td style="text-align: right; padding: 3px;">₹${(bill.extraCharges || 0).toFixed(2)}</td>
                    </tr>
                    <tr style="border-top: 2px solid #000;">
                        <td style="padding: 3px;"><strong>Total Amount:</strong></td>
                        <td style="text-align: right; padding: 3px;"><strong>₹${bill.totalAmount.toFixed(2)}</strong></td>
                    </tr>
                </table>
            </div>
    ...
    const opt = {
        margin: [10, 10, 10, 10],
        filename: `Estimate-${bill.billNumber}.pdf`,
